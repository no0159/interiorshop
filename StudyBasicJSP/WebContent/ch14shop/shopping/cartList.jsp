...
<%
	...
	if(session.getAttribute("id")==null) {
		...
		if(count == 0) {
%>
	<font color="white" font size="+1"><h3><b>장바구니</b></h3></font>
	<table align="center">
		<tr><td>장바구니에 담긴 물품이 없습니다.</td></tr>
	</table>
		<input type="button" value="쇼핑계속" onclick="javascript:window.location='list.jsp?book_kind=<%=book_kind %>'">
<%
		}else {
...
%>
			...
			<tr>
				...
				<td width="100">
				<%=NumberFormat.getInstance().format(cartList.getBuy_price()) %></td>
				<td width="150">
				...
				<%=NumberFormat.getInstance().format(cartList.getBuy_count()*cartList.getBuy_price()) %>
					<input type="button" value="삭제" onclick="javascript:window.location='cartListDel.jsp?list=<%=cartList.getCart_id() %>&book_kind=<%=book_kind %>'">
				</td>
			</tr>
<%} %>
			<tr>
				<td colspan="5" align="right" style="padding-right:35px;">
				<font color="blue"><b>총금액 : <%=NumberFormat.getInstance().format(total) %>
				</b></font></td>
			</tr>
			<tr>
				<td colspan="5"><input type="button" value="구매하기" 
					onclick="javascript:window.location='buyForm.jsp'">&nbsp;<input type="button" value="쇼핑계속" 
					onclick="javascript:window.location='list.jsp?book_kind=<%=book_kind %>'">&nbsp;<input type="button" value="장바구니 비우기" 
